...
static void set_scales(double winwidth, double winheight)   //设置全局变量
{
...
{
	return disc_size / g_disc_count * (g_disc_count-d);   //大小/铁盘数量*（铁盘数量-d）
}
...
static void draw_disc(double cx, double cy, int d)
{
	...
	SetPenColor("Blue");
	draw_rect(cx, cy, w, disc_height, 0);   //0就是不填充
}
//调用一个盘，画很多个盘。
// draw 铁盘
